...
double dist (CCPoint p1, CCPoint p2)               // 返回两点之间欧氏距离
{
    return p1.getDistance (p2);
}
bool equal_point (CCPoint p1, CCPoint p2)          // 判断两个点是否重合
{
...
*******************************************************************************/
double dotmultiply (CCPoint p1, CCPoint p2, CCPoint p0)
{
    return ((p1.x - p0.x)* (p2.x - p0.x) + (p1.y - p0.y)* (p2.y - p0.y));
}

/* 判断点p是否在线段l上，条件：(p在线段l所在的直线上)&& (点p在以线段l为对角线的矩形内) */
bool online (Segment l, CCPoint p)
{
...
CCPoint rotate (CCPoint o, double alpha, CCPoint p)
{
    CCPoint tp;
    p.x -= o.x;
    p.y -= o.y;
...
可以用于求线段之间的夹角
*/
double angle (CCPoint o, CCPoint s, CCPoint e)
{
...
*/
double relation (CCPoint p, Segment l)
{
    Segment tl;
    tl.s = l.s;
    tl.e = p;
...
// 求点C到线段AB所在直线的垂足 P
CCPoint perpendicular (CCPoint p, Segment l)
{
...
double ptolinesegdist (CCPoint p, Segment l, CCPoint &np)
{
    double r = relation (p, l);
    if (r<0)
    {
...
double ptoldist (CCPoint p, Segment l)
{
...
double ptopointset (int vcount, CCPoint pointset[], CCPoint p, CCPoint &q)
{
    int i;
...
bool CircleInsidePolygon (int vcount, CCPoint center, double radius, CCPoint polygon[])
{
    CCPoint q;
    double d;
    q.x = 0;
...
bool intersect_A (Segment u, Segment v)
{
...
bool intersect_l (Segment u, Segment v)
{
    return multiply (u.s, v.e, v.s)*multiply (v.e, u.e, v.s) >= 0;
}


bool IfIntersects (Segment& s1, Segment& s2)
{
    CCPoint& P1 = s1.s;
    CCPoint& P2 = s1.e;
    CCPoint& Q1 = s2.s;
...
bool intersects (CCPoint& a0, CCPoint& a1, CCPoint& b0, CCPoint& b1)
{
...
bool intersects (CCPoint& p1, CCPoint& p2, CCPoint& p3, CCPoint& p4, CCPoint & intersection)
{
...
// 求点p关于直线l的对称点
CCPoint symmetry (Line l, CCPoint p)
{
    CCPoint tp;
...
bool lineintersect (Line l1, Line l2, CCPoint &p) // 是 L1，L2
{
...
本程序默认第一个条件已经满足
*/
bool issimple (int vcount, CCPoint polygon[])
{
    int i, cn;
    Segment l1, l2;
    for (i = 0; i<vcount; i++)
...
void checkconvex (int vcount, CCPoint polygon[], bool bc[])
{
    int i, index = 0;
...
bool isconvex (int vcount, CCPoint polygon[])
{
    bool bc[MAXV];
...
double area_of_polygon (int vcount, CCPoint polygon[])
{
    int i;
    double s;
...
// 如果输入顶点按逆时针排列，返回true
bool isconterclock (int vcount, CCPoint polygon[])
{
    return area_of_polygon (vcount, polygon)>0;
}
// 另一种判断多边形顶点排列方向的方法
bool isccwize (int vcount, CCPoint polygon[])
{
    int i, index;
    CCPoint a, b, v;
    v = polygon[0];
...
如果点在多边形外： 返回2
*/
int insidepolygon (int vcount, CCPoint Polygon[], CCPoint q)
{
    int c = 0, i, n;
    Segment l1, l2;
...
bool InsideConvexPolygon (int vcount, CCPoint polygon[], CCPoint q) // 可用于三角形！
{
    CCPoint p;
    Segment l;
    int i;
...
void Graham_scan (CCPoint PointSet[], CCPoint ch[], int n, int &len)
{
    int i, j, k = 0, top = 2;
    CCPoint tmp;
...
void ConvexClosure (CCPoint PointSet[], CCPoint ch[], int n, int &len)
{
...
*/
bool LinesegInsidePolygon (int vcount, CCPoint polygon[], Segment l)
{
    // 判断线端l的端点是否不都在多边形内
...
CCPoint cg_simple (int vcount, CCPoint polygon[])
{
...
CCPoint gravitycenter (int vcount, CCPoint polygon[])
{
    CCPoint tp;
...
y坐标最大，最小的点肯定是凸顶点
*/
CCPoint a_point_insidepoly (int vcount, CCPoint polygon[])
{
    CCPoint v, a, b, r;
    int i, index;
    v = polygon[0];
...
*/
void pointtangentpoly (int vcount, CCPoint polygon[], CCPoint p, CCPoint &rp, CCPoint &lp)
{
    Segment ep, en;
    bool blp, bln;
    rp = polygon[0];
...
bool core_exist (int vcount, CCPoint polygon[], CCPoint &p)
{
    int i, j, k;
    Segment l;
    Line lineset[MAXV];
...
*/
bool point_in_circle (CCPoint o, double r, CCPoint p)
{
...
*/
bool cocircle (CCPoint p1, CCPoint p2, CCPoint p3, CCPoint &q, double &r)
{
...
int line_circle (Line l, CCPoint o, double r, CCPoint &p1, CCPoint &p2)
{
...
CCPoint rect4th (CCPoint a, CCPoint b, CCPoint c)
{
    CCPoint d;
...
内含： return 5；
*/
int CircleRelation (CCPoint p1, double r1, CCPoint p2, double r2)
{
...
// 调用ptoldist函数，在第4页
bool CircleRecRelation (CCPoint pc, double r, CCPoint pr1, CCPoint pr2, CCPoint pr3, CCPoint pr4)
{
...
// 两个点是否在直线同侧，是则返回true
bool SameSide (CCPoint p1, CCPoint p2, Line line)
{
...
// 两圆已经相交（相切）
void  c2point (CCPoint p1, double r1, CCPoint p2, double r2, CCPoint &rp1, CCPoint &rp2)
{
    double a, b, r;
    a = p2.x - p1.x;
    b = p2.y - p1.y;
...
// 必须保证相交
double c2area (CCPoint p1, double r1, CCPoint p2, double r2)
{
    CCPoint rp1, rp2;
...
//0----相离 1----相切 2----相交
int clpoint (CCPoint p, double r, double a, double b, double c, CCPoint &rp1, CCPoint &rp2)
{
    int res = 0;
...
void incircle (CCPoint p1, CCPoint p2, CCPoint p3, CCPoint &rp, double &r)
{
...
void cutpoint (CCPoint p, double r, CCPoint sp, CCPoint &rp1, CCPoint &rp2)
{
    CCPoint p2;
    p2.x = (p.x + sp.x) / 2;
    p2.y = (p.y + sp.y) / 2;
...
返回 –1： 左旋；
*/
int rotat (Segment l1, Segment l2)
{
