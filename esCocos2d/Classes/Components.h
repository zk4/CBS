...
class RayTraceComponents :public Component
{
    ...
    struct Ray {
        ...
        {
            if (s.ws.x == s.we.x && s.ws.y != s.we.y) { //与y轴垂直
                a = 1;
                b = 0;
                c = -s.ws.x;
            } else if (s.ws.x != s.we.x && s.ws.y == s.we.y) { //与x轴垂直
                a = 0;
                b = 1;
                c = -s.ws.x;
            } else {
                a = -(s.ws.y - s.we.y) / (s.ws.x - s.we.x);
                b = 1;
                c = -(s.ws.y*s.we.x - s.we.y*s.ws.x) / (s.we.x - s.ws.x);
            }
        }
        ...
        eConst intersect(Segment& w, CCPoint& intersection)
        {
            ...
            if (a*rd.x + b*rd.y == 0)return NOT_INTERSECTION;
            float T = (-a*rs.x - c - b*rs.y) / (a*rd.x + b*rd.y);


            float ix = rs.x + rd.x*T;
            float iy = rs.y + rd.y*T;
            float yy = a*ix+b*iy+c;
            intersection = ccp(ix,iy);
            CCPoint d1 = intersection - w.ws;
            CCPoint d2 = intersection - w.we;
            //in segment
            if (((d1.x*d2.x <0) || (d1.y*d2.y < 0) ) && T>=0)
...
public:
    ...
    void reConstructScanNodes()
    {
        edge_nodes.clear();
        for (auto &a : walls) {
            edge_nodes.push_back(a.ws);
            edge_nodes.push_back(a.we);
        }
    }
    ...
    bool HandleMessage(const Telegram& msg)
    {
        ...
        case    Telegram_DRAW: {
            ...
            for (auto & l: lightWallSegments) {
                if (l.second.size()>0)
                    ccDrawLine(l.second[0], _light);
                else
                    ccDrawLine(_light, l.first );
            }
